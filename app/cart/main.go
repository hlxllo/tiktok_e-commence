...
import (
	...
	"log"
	"tiktok_e-commence/app/cart/biz/api"
	"tiktok_e-commence/app/cart/biz/dal"
	"tiktok_e-commence/app/cart/biz/server"
...
func main() {
	...
	{
		cartGroup.POST("", api.AddItemHandler(client))
		cartGroup.GET("", api.GetCartHandler(client))
		cartGroup.DELETE("", api.EmptyCartHandler(client))
	}
	// 启动 Gin 服务
